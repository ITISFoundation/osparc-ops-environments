...
stacks = {"master" : {"key": "osparc-infra/docs/deployments-hardware-and-credentials/osparc.speag.com/osparc-speag.pem", "e2e-name" : "Master", "sparc-external-name": "master", "registry" : "registry.osparc-master.speag.com"},
"aws-staging" : {"key": "osparc-infra/docs/deployments-hardware-and-credentials/staging.osparc.io/osparc-staging.pem", "e2e-name" : "AWS-Staging", "sparc-external-name": "aws-staging", "registry" : "registry.staging.osparc.io" },
"aws-production" : {"key": "osparc-infra/docs/deployments-hardware-and-credentials/osparc.io/osparc-production.pem", "e2e-name" : "AWS-Production", "sparc-external-name": "aws-production", "registry" : "registry.osparc.io" },
"tip" : {"key": "osparc-infra/docs/deployments-hardware-and-credentials/tip.itis.swiss/osparc-public.pem", "e2e-name" : "tip.itis.swiss", "sparc-external-name": "tip-public", "registry" : "registry.tip.itis.swiss" },
"osparc-staging.speag.com" : {"key": "osparc-infra/docs/deployments-hardware-and-credentials/osparc.speag.com/osparc-speag.pem", "e2e-name" : "Dalco", "sparc-external-name": "dalco-staging-production", "registry" : "registry.osparc-staging.speag.com" },
"osparc.speag.com" : {"key": "osparc-infra/docs/deployments-hardware-and-credentials/osparc.speag.com/osparc-speag.pem", "e2e-name" : "Dalco", "sparc-external-name": "dalco-staging-production", "registry" : "registry.osparc.speag.com" },}
...
# This function parse yaml data from sparc-external file and return the versions of sim4life that needs to be pulled.
def yaml_data_to_pulling_info(stack_arg):
    global stacks
    stack = stacks[stack_arg]
    pull_images = []
    images_names = ["ci/osparc-osparc-s4l/master/s4l-core-lite", "ci/osparc-osparc-s4l/master/sym-server-dy"] 
                ...
                if image["from"]["repository"] == sim4life_image:
                    for destinations in image["to"]:
                        if destinations["destination"] == stack["sparc-external-name"]:
                            ...
                            pull_images.append(destinations["repository"] + ":" + tag)
    return pull_images
        ...
        try:
            ...
            if not_found_str not in output:
                ...
                for image in images_to_pull:
                    ...
                    stdin, stdout, stderr = ssh.exec_command("docker pull " + stack["registry"] + "/" + image )
...
def list_aws_instances(stack_arg):
    ...
    ec2 = boto3.client('ec2', aws_access_key_id=os.getenv('AWS_ACCESS_KEY'), aws_secret_access_key=os.getenv('AWS_SECRET_KEY'), region_name="us-east-1")
    response = ec2.describe_instances(Filters=[{'Name': 'tag:Name', 'Values': [message]}])
...
def clone_repos():
    ...
    # Clone osparc-infra (413), e2e-ops (525), and sparc-external (258)
    for id in [413, 525, 258]:
        project_id = id
...
def clean_repos():
    for repos in ["osparc-infra", "e2e-ops", "sparc-external" ]:
...
def hosts_list(stack_arg):
    ...
    if stack_arg == "aws-production" or stack_arg == "aws-staging":
        ...
        for host in aws_hosts:
            hosts.append({'IP': host, 'Username': hosts[0]["Username"], 'IdentityFile': hosts[0]["IdentityFile"], 'SudoPassword': hosts[0]["SudoPassword"]})
    return hosts
