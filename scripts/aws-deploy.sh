...
# If the simcore config isn't updated with the proper domain configuration, we need to update it and to push it to gitlab. The script stop

# -------------------------------- Simcore -------------------------------

pushd ${repo_basedir}/services/simcore;
if grep -q MONITORING_DOMAIN=.$MACHINE_FQDN .env && grep -q PUBLISHED_HOST_NAME=$MACHINE_FQDN .env && grep -q REGISTRY_URL=$REGISTRY_DOMAIN .env
then
    echo ".env in Simcore ready for deployment"
else
    ...
    echo "Some values weren't up to to date for AWS in Simcore .env. They have been updated, please push it to gitlab and restart the script"
    exit
fi
