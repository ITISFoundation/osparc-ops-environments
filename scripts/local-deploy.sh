...
if [[ $current_git_url == git* ]]; then
    ...
    $psed -i -e "s|https://github.com/ITISFoundation/osparc-ops.git|https://github.com/$current_organisation/osparc-ops.git|" deployment_config.default.yaml
else
    $psed -i -e "/- id: simcore-ops-repo/ { n; s|url:.*|url: $current_git_url|; }" deployment_config.default.yaml
fi
$psed -i -e "/- id: simcore-ops-repo/ { n; n; s|branch:.*|branch: $current_git_branch|; }" deployment_config.default.yaml
...
$psed -i -e "s/environment: {}/$YAML_STRING/" deployment_config.default.yaml
# update
$psed -i -e "s/S3_ENDPOINT:.*/S3_ENDPOINT: ${MACHINE_FQDN}:10000/" deployment_config.default.yaml
$psed -i -e "s/S3_ACCESS_KEY:.*/S3_ACCESS_KEY: ${SERVICES_PASSWORD}/" deployment_config.default.yaml
$psed -i -e "s/S3_SECRET_KEY:.*/S3_SECRET_KEY: ${SERVICES_PASSWORD}/" deployment_config.default.yaml
# portainer
$psed -i -e "/- url: .*portainer:9000/ { n; s/username:.*/username: ${SERVICES_USER}/; }" deployment_config.default.yaml
$psed -i -e "/- url: .*portainer:9000/ { n; n; s/password:.*/password: ${SERVICES_PASSWORD}/; }" deployment_config.default.yaml
# extra_hosts
$psed -i -e "s|extra_hosts: \[\]|extra_hosts:\n        - \"${MACHINE_FQDN}:${machine_ip}\"|" deployment_config.default.yaml
# update
$psed -i -e "/extra_hosts:/ { n; s/- .*/- \"${MACHINE_FQDN}:${machine_ip}\"/; }" deployment_config.default.yaml
make down up;
popd
