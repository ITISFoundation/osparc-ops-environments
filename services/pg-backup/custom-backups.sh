...
function backup_db() {
  ...
  for DB in ${DBLIST}; do
    ...
    if [ -z "${DB_TABLES:-}" ]; then
      ...
      echo "Backing up $FILENAME done"
      if [[ ${STORAGE_BACKEND} == "S3" ]]; then
        gzip $FILENAME
        echo "Backing up $FILENAME to s3://${BUCKET}/"
        ${EXTRA_PARAMS}
...
if [[ ${STORAGE_BACKEND} == "S3" ]]; then
  ...
  echo "Sync globals.sql to ${BUCKET} bucket  "
...
if [ "${REMOVE_BEFORE:-}" ]; then
  ...
  if [[ ${STORAGE_BACKEND} == "FILE" ]]; then
    ...
    find ${MYBASEDIR}/* -type f -mmin +${TIME_MINUTES} -delete
