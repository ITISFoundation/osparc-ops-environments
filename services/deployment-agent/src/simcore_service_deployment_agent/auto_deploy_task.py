...
async def update_portainer_stack(app_config: Dict, app_session: ClientSession, stack_cfg: Dict):
    ...
    for config in portainer_cfg:
        ...
        if not current_stack_id:
            ...
            swarm_id = await portainer.get_swarm_id(url, app_session, bearer_code, config["endpoint"])
            await portainer.post_new_stack(url, app_session, bearer_code, swarm_id, config["endpoint"], config["stack_name"], stack_cfg)
        else:
            log.debug("updating the configuration of the stack...")
            await portainer.update_stack(url, app_session, bearer_code, current_stack_id, config["endpoint"], stack_cfg)
